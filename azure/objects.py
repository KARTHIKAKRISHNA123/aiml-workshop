...
result = client.analyze_from_url(image_url, visual_features=[VisualFeatures.OBJECTS])
print("Image analyzed successfully")
print("Objects Found: ")
print(len(result.objects.list))

for obj in result.objects.list:
    # Each object has tags (list) and bounding_box
    object_tags = ", ".join([f"{tag.name} ({tag.confidence:.2f})" for tag in obj.tags])
    print(f" - {object_tags} at location {obj.bounding_box}")
